...
const JWT_SECRET = process.env.JWT_SECRET || 'slave-game-secret';
...
function dealCards(R) {
  ...
  R.players.forEach((p, i) => {
    R.hands[p.name] = sortHand(deck.filter((_,j) => j % 4 === i));
  });
}
...
// ── Card swap (Round 2 prep) ───────────────────────────────────────────────────
function broadcastSwap(R) {
  ...
  R.players.filter(p => p.socketId && !p.isBot).forEach(p => {
    ...
    if (kingEntry?.name === p.name) {
      data.mustPick  = 2;
      data.receiving = slaveGiving;
      data.submitted = !!R.swapGiving[p.name];
    } else if (queenEntry?.name === p.name) {
      data.mustPick  = 1;
      data.receiving = comGiving;
      data.submitted = !!R.swapGiving[p.name];
...
function executeSwap(R) {
  ...
  // King gives chosen 2 → Slave; Slave gives 2 highest → King
  ...
  // Queen gives chosen 1 → Commoner; Commoner gives 1 highest → Queen
...
// ── Core game logic ───────────────────────────────────────────────────────────
function doPlay(R, playerName, cards) {
  ...
  if (finished) {
    ...
    // ── Coup rule: if R2 Slave finishes first → R1 King is demoted immediately ──
...
function doPass(R, playerName) {
  ...
  if (allOthersPassed(R)) {
    ...
    R.curPlay    = null;
    R.passedIds  = new Set();
    R.tableCards = []; // trick won — clear table for new lead
...
function endRound(R) {
  // Append couped player at the very end (last = slave)
  if (R.coupedPlayer && !R.finishOrder.includes(R.coupedPlayer)) {
    R.finishOrder.push(R.coupedPlayer);
  }
  assignRanks(R);
...
// ── Bot takeover on disconnect (75%) ──────────────────────────────────────────
function clearTimer(R, name) {
  clearTimeout(R.autoPassTimers[name]);
  delete R.autoPassTimers[name];
}
function scheduleAutoPass(R, name) {
  clearTimer(R, name);
  R.autoPassTimers[name] = setTimeout(() => {
    ...
    if (!['roundEnd','gameOver'].includes(R.phase)) {
      scheduleBots(R);
      if (currentPlayerName(R) === name) scheduleAutoPass(R, name);
    }
  }, BOT_TAKEOVER_MS);
}
...
function scheduleBots(R) {
  ...
  if (next.isBot || next.disconnected) {
    setTimeout(() => runBot(R, next.name, !!next.isBot), next.isBot ? BOT_THINK : 0);
  }
}
function runBot(R, name, isBot) {
  if (R.phase !== 'playing' || currentPlayerName(R) !== name) return;
  const hand = R.hands[name];
  if (!hand?.length) return;
  const indices = findBotCards(hand, R.curPlay);
  const result  = indices ? doPlay(R, name, indices.map(i=>hand[i])) : doPass(R, name);
  if (!result.ended) scheduleBots(R);
}
...
function startGame(R, round) {
  R.round     = round;
  R.turnOrder = R.players.map(p => p.name);
  resetForRound(R);
  setTurnTo(R, findFirstPlayer(R));
  R.phase = 'playing';
...
setInterval(() => {
  for (const R of rooms.values()) {
    ...
    const next = R.players.find(p => p.name === currentPlayerName(R));
    if (next?.isBot) runBot(R, next.name, true);
  }
}, 3000);
...
io.on('connection', socket => {
  ...
  socket.on('joinRoom', rawId => {
    ...
    broadcastRoomList();
    emitLobby(R);
    if (R.players.filter(p=>!p.isBot).length === 4) startGame(R, 1);
  });
  ...
  socket.on('play', indices => {
    ...
    const r = doPlay(R, p.name, cards);
    if (!r.ok) socket.emit('invalid', r.msg);
    else if (!r.ended) scheduleBots(R);
  });
  ...
  socket.on('pass', () => {
    ...
    const p = R.players.find(p => p.socketId === socket.id);
    if (!p || currentPlayerName(R) !== p.name) return;
    const r = doPass(R, p.name);
    if (!r.ok) socket.emit('invalid', r.msg);
    else scheduleBots(R);
  });

  // Round 2: deal cards → cardSwap phase
  socket.on('startRound2', () => {
    const R = getRoom(socket);
    if (!R || R.phase !== 'roundEnd') return;

    dealCards(R); // fresh deal for R2

    const kingEntry  = R.round1Ranks?.find(r => r.rankLabel === RANK_NAMES[0]);
    const queenEntry = R.round1Ranks?.find(r => r.rankLabel === RANK_NAMES[1]);
    R.swapPending = new Set();
    R.swapGiving  = {};
  ...
  // King/Queen submits swap choice
  socket.on('submitSwap', indices => {
    ...
    if (R.swapPending.size === 0) {
      executeSwap(R);
    } else {
      broadcastSwap(R); // update submitted status for others
    }
  });
  ...
  socket.on('requestState', () => {
    const R = getRoom(socket);
    if (!R) return;
    if (['playing','roundEnd','gameOver'].includes(R.phase)) broadcast(R);
    else if (R.phase === 'lobby') emitLobby(R);
    else if (R.phase === 'cardSwap') broadcastSwap(R);
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (R.phase === 'lobby') {
      ...
      if (p) {
        ...
        if (R.phase === 'playing' && currentPlayerName(R) === p.name) scheduleAutoPass(R, p.name);
        if (R.phase === 'playing') broadcast(R);
      }
    }
  });
});
